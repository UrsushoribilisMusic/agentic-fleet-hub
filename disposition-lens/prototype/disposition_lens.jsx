      ...
      case "smirk": // Confident: gentle smile with slight tongue peek
        return (
          <g>
            {tongueOut > 0.08 && (
              <path
                d={`M144,${my + 2} Q150,${my + 10 + tongueOut * 8 + flutter * 3} 156,${my + 2} C158,${my + 8 + tongueOut * 8} 154,${my + 12 + tongueOut * 8} 150,${my + 12 + tongueOut * 8} C146,${my + 12 + tongueOut * 8} 142,${my + 8 + tongueOut * 8} 144,${my + 2} Z`}
                fill="#FF6584"
                stroke="#D84A68"
                strokeWidth="1"
              />
        ...
        return (
          <g>
            ...
            <line
              ...
              y1={my + 4}
              x2="150"
              y2={my + 22 + tongueOut * 12 + flutter * 4}
              stroke="#B83250"
              strokeWidth="1.4"
              strokeLinecap="round"
            />
    ...
    <g>
      ...
      <g transform={`rotate(${headTilt} 150 155)`}>
        ...
        <g transform={`translate(96 ${112 + earY}) rotate(${earL}) translate(-96 -112)`}>
          ...
          />
          <path
            d="M89,122 C73,109 60,132 66,159 C70,174 84,170 90,149 C93,137 93,127 89,122 Z"
            fill={p.tint}
            opacity="0.35"
          />
        </g>
        ...
        {/* Right Ear */}
        <g transform={`translate(204 ${112 + earY}) rotate(${earR}) translate(-204 -112)`}>
          <path
          ...
          />
          <path
            d="M211,122 C227,109 240,132 234,159 C230,174 216,170 210,149 C207,137 207,127 211,122 Z"
            fill={p.tint}
            opacity="0.35"
          />
        </g>
...
function DispositionLens() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      c.headTilt = lerp(c.headTilt || 0, target.headTilt || 0, k);
      c.earL = lerp(c.earL || 0, target.earL || 0, k);
      c.earR = lerp(c.earR || 0, target.earR || 0, k);
      c.earY = lerp(c.earY || 0, target.earY || 0, k);
      ...
      g.lookY = lerp(g.lookY, g.tLookY, gk);
      ...
      const rp = {
        ...
        pupilY: c.pupilY + (idling ? g.lookY : 0),
        headTilt: (c.headTilt || 0) + (idling ? g.tilt : 0),
        earL: (c.earL || 0) - (idling ? g.ear * 7 : 0),
        earY: (c.earY || 0) - (idling ? g.ear * 2 : 0),
      };
      setRender({ ...rp, mouth: target.mouth, bob, speakPhase });
  ...
  useEffect(() => {
    let alive = true;
    const g = idleG.current;
    const settle = () => { g.tLookX = 0; g.tLookY = 0; g.tTilt = 0; g.tEar = 0; };
    const tick = () => {
      if (!alive) return;
      if (targetKey === "idle" && !reduceMotion.current) {
        settle();
      ...
      setTimeout(tick, 2600 + Math.random() * 2800);
    };
    const id = setTimeout(tick, 2200);
    return () => { alive = false; clearTimeout(id); };
  }, [targetKey]);
