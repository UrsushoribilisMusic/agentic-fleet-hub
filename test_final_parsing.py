#!/usr/bin/env python3
"""Test the final parsing function with fixed patterns"""

import re

def parse_mission_control_final():
    """Final test parsing MISSION_CONTROL.md for ticket status."""
    ...
    print(f"Parsed {len(open_tickets)} open tickets:")
    for ticket in open_tickets:
        print(f"  Ticket #{ticket['id']}: {ticket['description'][:50]}... (Owner: {ticket['owner']}, Status: {ticket['status']})")
    
    # Extract CLOSED tickets - fixed pattern to handle ranges and special IDs
    closed_section_match = re.search(
        r"### CLOSED\s*\n((?:- \*\*[^\*]+\*\*: .+? \n)+)",
        content
    )
    ...
    if closed_section_match:
        ...
        for line in closed_list.strip().split("\n"):
            if line.strip():
                # Match patterns like "- **#1-#6**: description" or "- **#10**: description" or "- **#s4znd9zm9a2rr0v**: description"
...
if __name__ == "__main__":
    result = parse_mission_control_final()
