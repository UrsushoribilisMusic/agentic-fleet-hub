...
| **#67** | Update setup wizard for v0.2.0 features | Codi | todo | BLOCKER for #53. Add wizard steps: (1) Deployment scenario selector (Local/VPS/Hybrid — Scenario 3 shows "contact us" note). (2) Telegram bridge config step: TELEGRAM_TOKEN + TELEGRAM_CHAT_ID + bot setup link, skippable. (3) OpenClaw step: opt-in gateway URL, skippable (ties into #59). (4) GitHub sync step: GITHUB_TOKEN for Issues↔PB sync, skippable (ties into #57). Update setup-lib.mjs buildSetupPayload + fleet_meta.json installation schema to persist new fields. |
