...
| **pc019home** | PC-019 [P0]: Home screen redesign — Daily Note as hero element | codi | open | Redesign Home tab: Daily Note hero (full-width, today date + inferred location), Recent Captures (last 5 cards), Smart Space previews (top 3 pinned saved searches with live counts), On This Day card. All from real SQLite. Renders within 1s of foreground. 8pts. |
| **pc020dnot** | PC-020 [P0]: Daily Note — auto-population from device data | clau | open | DailyNoteService.today(): photos taken today (PHPhotoLibrary), calendar events (EventKit), health snapshot (HealthKit steps + sleep). Free-text Markdown area persists as daily-note Card in SQLite keyed by date. Reverse geocode location (cache per day). 8pts. |
| **pc021otd0** | PC-021 [P1]: Daily Note — On This Day historical recall | clau | open | OnThisDayService.fetch(for: Date): query cards/photos from same calendar day in prior years. Return 1 result (photo with GPS > daily-note > any card). Compact tappable card at bottom of Home. Hidden if no historical content. 3pts. |
| **pc022tags** | PC-022 [P0]: Unified hashtag index — manual + auto tags in one system | gem | open | HashtagParser.extract(from: String) finds all #word tokens. Tags stored normalised (lowercase, no #) in SQLite tags table with source (auto/manual/system/vision). Runs on every card save/edit. Existing PC-018 auto-tags migrated. Tapping tag chip triggers filtered search. 5pts. |
| **pc023cld0** | PC-023 [P1]: Hashtag browser and tag cloud in Library | gem | open | Tags section in Library showing top 20 tags sized by frequency (log scale, 14–28pt). Tapping opens filtered Library view titled '#tagname — N items'. Updates in real time. 5pts. |
| **pc024auto** | PC-024 [P1]: Hashtag autocomplete in note editor and capture | misty | open | Inline autocomplete dropdown after typing # + 1 char. Top 5 matching tags by frequency, case-insensitive. Works in daily note editor, card note editor, capture title field. Selecting inserts tag and closes dropdown. 3pts. |
| **pc025srch** | PC-025 [P0]: Save a search — create Smart Space from Search tab | clau | open | SmartSpace model: {id, name, query, tags, createdAt, lastRunAt, resultCount}. 'Save this search' button after any search. Bottom sheet: name + tag (default #queries). Appears in Library as card with live result count (re-queried on display, 500ms debounce). 5pts. |
| **pc026mgmt** | PC-026 [P1]: Smart Space management — edit, delete, pin to Home | gem | open | Long-press Smart Space card → context menu: Edit, Delete, Pin to Home. Edit opens prefilled bottom sheet. Delete removes SmartSpace record only. Pin to Home marks as featured (up to 3 shown on Home). 3pts. |
| **pc027alrt** | PC-027 [P2]: Smart Space alert mode — notify on new match | misty | open | SmartSpace gains alertEnabled: Bool. Toggle in edit sheet. SmartSpaceAlertService checks all alert-enabled spaces on card save (async, non-blocking, batch). Local UNUserNotificationCenter notification on match. 5pts. |
| **pc028peop** | PC-028 [P0]: People — face cluster display and naming | codi | open | PeopleService.fetchClusters() via PHAssetCollection people albums. Grid of circular avatars (most recent face photo as thumbnail). Tap name to rename inline (stored in SQLite linked to PHAssetCollection localIdentifier). Sorted by photo count desc. SPIKE the API on device before building UI. 8pts. |
| **pc029pdet** | PC-029 [P0]: Person Detail — photo timeline + contact card + CRM layer | codi | open | Photo timeline (PHAssets from cluster grouped by trip/month), contact match (fuzzy name against indexed contacts), linked cards (tags include person name or [[PersonName]] backlink), interaction log (EventKit events with person name). '+' button adds note pre-tagged with #personname. 8pts. |
| **pc030blnk** | PC-030 [P1]: Person card — backlink integration | clau | open | Register Person cards as [[PersonName]] backlink targets. Autocomplete includes Person names. Tapping [[PersonName]] in a note navigates to Person Detail. Person Detail 'Mentioned in' shows all notes with that backlink. 3pts. |
| **pc031proj** | PC-031 [P1]: Project model and creation | codi | open | Project is a Card of type 'project': {id, name, description, coverImageAssetId?, tags, createdAt, cardCount}. Create via Library '+'. Full-width card in Library. Long-press any card → Add to Project (1 project max). Project Detail: card list sorted by createdAt. No Canvas. 5pts. |
| **pc032dash** | PC-032 [P1]: Project Dashboard — scoped Daily Note and saved searches | misty | open | Project Detail two tabs: Cards and Dashboard. Dashboard: project-scoped daily note (Card type daily-note with projectId, keyed by projectId+date), Smart Spaces tagged #project-name, quick capture button (opens Capture with project pre-selected). 5pts. |
| **pc033writ** | PC-033 [P2]: Creative Writing mode — long-form note with chapter outline | misty | open | Long-form mode toggle in Project note editor toolbar. Outline panel from H1/H2 headings as bottom drawer. Tapping heading scrolls to section. Export as .md via share sheet. Word count in toolbar. 5pts. |
| **pc034mbrd** | PC-034 [P1]: Mood Board view — image-only Library layout | misty | open | View toggle in Library top-right (Grid / Mood Board). Mood Board: 2-col square thumbnails, no text/chips, PHAsset localIdentifier loading (target 400x400pt @2x). Respects active tag filter. Tapping thumbnail opens card detail. Preference persists per-session. 5pts. |
| **pc035nmbd** | PC-035 [P1]: Named Mood Boards — curated image collections | gem | open | Mood Board = Project with type 'mood-board'. Create via Library '+'. Opening auto-applies Mood Board layout. Intelligence row gains 'Boards' icon with count. Add images via #moodboard-name tag or long-press. Reuses Project model entirely. 3pts. |
