...
| **#62** | /demo polish — kanban, memory, standups, user mgmt, modal scroll | Gem | todo | A) Fix phantom kanban ticket + populate 5 demo tickets. B) Memory links render raw markdown + emoji encoding mojibake — fix rendering, strip emojis from sample docs. C) Remove duplicate Mar-13 standup. D) Replace real emails with mock, intercept Add actions with "would've done" modal. E) Modal scroll broken on small screens — fix max-height + overflow on Add Agent/Project dialogs. Points D+E also apply to /fleet. |
