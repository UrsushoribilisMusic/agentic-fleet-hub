...
| **#57** | GitHub Issues ↔ PocketBase two-way sync | Clau | peer_review | Implemented: github_sync.py running as fleet.github launchd service. Outbound+inbound sync live. Bug fix: close_approved_issues() added (approved filter excluded them). Script+plist synced to repo. Needs peer approval. |
...
| **#67** | Update setup wizard + Doctor for v0.2.0 features | Codi | todo | BLOCKER for #53. A) Wizard steps: (1) Deployment scenario selector (Local/VPS/Hybrid). (2) Telegram config: TELEGRAM_TOKEN + TELEGRAM_CHAT_ID, skippable. (3) OpenClaw: opt-in gateway URL, skippable (ties into #59). (4) GitHub sync: GITHUB_TOKEN, skippable (ties into #57). B) setup-lib.mjs: extend buildSetupPayload + getDefaultFleetMeta + normalizeFleetMeta with new fields. C) runDoctor: add checks for new fields — TELEGRAM_TOKEN present if telegram enabled, dispatcher.py + telegram_bridge.py in scripts/, PocketBase binary referenced if scenario != local. Doctor checks must mirror wizard collection. D) Dry-run verification: run `npx create-flotilla` locally against the package, confirm dashboard (engineering/index.html + assets/) loads correctly, all sections render, dark/light mode works, Add Agent modal submits. Must pass before #53 can publish. |
