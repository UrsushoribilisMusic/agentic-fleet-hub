...
# Files that trigger a full startup when changed
WATCHED_FILES = [
    "MISSION_CONTROL.md",
...
def main():
    ...
    current = {f: _checksum(os.path.join(repo, f)) for f in WATCHED_FILES}

    # ── Step 2: compare to previous run ───────────────────────────────────────
    cache    = _load_cache(cache_path)
    previous = cache.get("checksums", {})
    changed  = [f for f in WATCHED_FILES if current.get(f) != previous.get(f)]
    ...
    if "MISSION_CONTROL.md" in changed:
        if _ticket_for_agent(os.path.join(repo, "MISSION_CONTROL.md"), aliases):
            reasons.append("open ticket assigned to you")

    if "AGENTS/MESSAGES/inbox.json" in changed:
        if _inbox_for_agent(os.path.join(repo, "AGENTS/MESSAGES/inbox.json"), aliases):
